...
<div class="task">
<div class="task-top"><div class="task-title">8.2 — Visibilidad de marca: no aparece buscando "Base Core" solo</div><span class="chip progress"><span class="dot"></span>En progreso · #1, #2 y #4 confirmados por Mariano</span></div>
...
<p><strong>Implementado (14/9):</strong> Mariano confirmó avanzar con #1, #2 y #4 (invisibles/sin riesgo, sin dependencias). #1 y #2 ya están en <a href="https://github.com/marianosandonato/basecoreweb/pull/41" target="\_blank" rel="noopener">PR #41</a> — <code>alternateName: "Base Core"</code> en el JSON-LD <code>ProfessionalService</code> y la línea "Also known as: Base Core" en <code>/llms.txt</code> — verificado con tsc/eslint/build y con <code>next start</code> local (JSON-LD y `/llms.txt` confirmados en el HTML real), en preview de Vercel pendiente de revisión de Mariano antes de mergear. #4 (chequeo mensual de la query "base core") queda establecido junto a 5.1/6.5; primera lectura de referencia (14/9, <code>gsc.py analytics --days 90 --query "base core"</code>): 30 impresiones en Home (posición 3.7), 2 en /contacto (9.5) y 3 en /en (8.3) — línea de base para comparar en el próximo chequeo.</p>
<p><strong>Sin implementar, no requieren acción ahora:</strong> #3 (bajar expectativa, ya incorporado al análisis), #5 y #6 (dependen de 4.1 y 4.3, bloqueadas), #7 (mención en copy visible — Mariano no confirmó todavía, queda pendiente de decisión aparte).</p>
...
<ul>
<li><strong>8.2 (visibilidad de marca "Base Core" en buscadores):</strong> #1, #2 y #4 confirmados por Mariano — #1/#2 en <a href="https://github.com/marianosandonato/basecoreweb/pull/41" target="\_blank" rel="noopener">PR #41</a> esperando su revisión en preview de Vercel antes de mergear; #4 establecido como chequeo recurrente. #7 sigue sin confirmar.</li>
...
<footer class="note">
Última actualización: 2026-09-14 (Fase 8 "Base Core en motores de búsqueda": 8.1 naming en pausa, 8.2 con #1/#2 (alternateName + llms.txt) en PR #41 pendiente de revisión y #4 (chequeo mensual) establecido; antes, el mismo día, se resolvió el acceso a GA4 para 1.25 y se cerraron los 6 hallazgos de la auditoría del 13/9) · se irá marcando como Hecho a medida que avancemos.
</footer>
</div>
</body></html>