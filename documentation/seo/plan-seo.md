...
**Primera lectura del código (24/9, sin cambios todavía):** en `src/components/Turnstile.tsx`, el temporizador de 12 segundos (`STUCK_TIMEOUT_MS`) **no se cancela cuando la verificación sale bien**. Si Turnstile verifica en 3 segundos, a los 12 el temporizador igual marca el widget como colgado y muestra el cartel, con el ✓ ya visible al lado. Coincide con el síntoma de desktop. Además, el temporizador arranca cuando se carga la página y no cuando el widget empieza a verificar: el widget se carga recién cuando el formulario se acerca a la pantalla, así que en las páginas donde el formulario está abajo (Home, páginas de servicio) el cartel puede aparecer antes de que el captcha siquiera exista. Las dos cosas afectan también a mobile y a los dos formularios (`ContactForm.tsx` y `EbookForm.tsx`, que comparten el componente).

**Falta precisar el caso mobile:** qué celular y navegador, en qué página, y qué se ve exactamente (el widget no carga, queda "verificando", aparece el cartel, el botón de enviar sigue deshabilitado o el envío da error). Puede ser el mismo bug del temporizador o algo distinto, como el conflicto con iCloud Private Relay del 18/9.

**Plan:** 1) reproducir los dos síntomas contra un build local, en desktop y en mobile emulado; 2) corregir el temporizador: cancelarlo cuando llega la verificación y arrancarlo recién cuando el widget se monta; 3) con los datos de Mariano, reproducir y resolver el caso mobile; 4) preview de Vercel y prueba real de Mariano en desktop y en su iPhone, antes de producción.
...
| 1.37 | Turnstile: cartel de error falso en desktop y fallas en mobile (reabierta 24/9) | Pendiente |
