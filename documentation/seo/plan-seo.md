...
> - Nota: el widget de revisión interactiva (OK/No/Sin revisar) que este documento tuvo brevemente se sacó — duplicaba el archivo a más del doble de su tamaño y dejaba una línea de más de 98.000 caracteres, imposible de leer para verificar conflictos entre sesiones (forzó un publish con `force` en otra sesión). El documento vuelve a ser HTML estático, como siempre fue — el veredicto de revisión de un hallazgo se registra por chat, no por un widget en el artifact. 1.40 (overlay de /marketing) cerrada del todo tras 3 rondas — la causa real era el color de la foto, no el overlay; detalle completo en el Historial Técnico SEO.
...
75 / 87 tareas · 2 en progreso (1.25, 8.2) · +4 bloqueadas (4.1, 4.3, 4.4, 4.5)
