...
> - Nota: esta sesión cerró 1.25 (INP real de campo, 76/87 tareas) y cerró la ronda del 21/9 de 1.28 — segunda regresión de LCP mobile en PSI, esta vez confirmada como ruido de PSI (no del código) con un A/B limpio de Lighthouse local contra el commit del 18/9.
...
76 / 87 tareas · 1 en progreso (8.2) · +4 bloqueadas (4.1, 4.3, 4.4, 4.5)
...
Pendiente, tarea recurrente · objetivo de mobile +90 cerrado el 21/9 sin perseguirlo más (Mariano)
...
**Ronda del 21/9 — nueva señal de LCP mobile alto en PSI, investigada y desestimada de nuevo: es ruido de PSI, no una regresión real. Esta vez con evidencia definitiva, no solo repetir corridas contra producción.**

5 corridas frescas de PSI contra producción (cache-busting), Home mobile:

```
1: Performance 73 · LCP 4.7s · TBT 110ms
2: Performance 72 · LCP 5.6s · TBT 100ms
3: Performance 72 · LCP 5.5s · TBT 120ms
4: Performance 74 · LCP 5.3s · TBT  80ms
5: Performance 77 · LCP 4.9s · TBT  40ms
Desktop: Performance 97 · LCP 1.2s (en línea con baseline)
```

Las 5 corridas dieron LCP 4.7-5.6s — peor que el rango post-fix del 18/9 (3.9-4.7s) y sin ningún valor bajo que sugiriera ruido puro, a diferencia de la vez anterior. TTFB (~0.04-0.13s) y `x-vercel-cache: HIT` constantes por curl — descartado problema de servidor. El desglose crudo de la API de PSI (`lcp-breakdown-insight`) mostró que el 83% del tiempo de LCP es **"Element render delay"** (~2000ms de ~2400ms totales) — no carga de red. El main-thread-work-breakdown solo sumaba ~700ms simulados (GTM 161ms + 2 chunks de la app ~245ms + resto), sin explicar del todo el delay — ambigüedad real que ameritaba una prueba más rigurosa que repetir corridas contra producción (que trae variables de red/infra de PSI que no se pueden controlar).

**A/B limpio con Lighthouse local (13.5.0, vía `npx`), mismo throttling mobile simulado, sin red/servidor de por medio:** se armó un `git worktree` del commit `a00ddcc` (baseline post-fix del 18/9, mismo `package-lock.json` que HEAD — sin diffs de dependencias, `node_modules` copiado en vez de symlinkeado porque Turbopack rechaza symlinks fuera del filesystem root), build de producción + `next start` de los dos (baseline en :4001, HEAD/`8dc2b56` en :4002), 3 corridas de Lighthouse contra cada uno.

```
Baseline 18/9 (a00ddcc), local: LCP 4.5 / 4.7 / 4.2s (avg 4.47s) · Performance 83/83/85
HEAD 21/9 (8dc2b56), local:     LCP 4.0 / 4.7 / 4.4s (avg 4.37s) · Performance 86/83/84
```

**Conclusión: los rangos se superponen del todo — HEAD está igual o levemente mejor que el baseline del 18/9, no peor.** Los 3 commits de código entre el 18/9 y hoy que tocan lo que Home renderiza (`4561e89`/`9ef3ca8`: flip cards, `useFlipTeaser.ts` +72 líneas; `f2fdf02`/`c477c71`: overlay de `PageHero.tsx`, que Home ni siquiera usa) no causaron ninguna regresión real de LCP. Los 4.7-5.6s que mostró PSI contra producción son ruido de la infraestructura de PSI (carga del lado de Google, variación de red hacia Vercel) — mismo patrón que la regresión desestimada el 18/9, ahora confirmado con una prueba que aísla la variable de código del todo. Entorno de prueba limpiado (worktree y servidores locales eliminados) sin dejar residuo.

**Ajuste al criterio de esta tarea:** cuando una ronda de PSI contra producción dé señal ambigua (ej. desglose crudo sin explicación clara desde main-thread-work), preferir un A/B local con Lighthouse (aísla la variable de código de la de red/infra de PSI) antes de invertir tiempo en atribuir causa a commits específicos solo con corridas contra producción.

**Ronda del 21/9 (2), a pedido de Mariano — objetivo explícito: mobile +90 (paridad con desktop, ya en 97+). Dos fixes implementados y verificados, con resultado honesto: mejora real pero modesta, no alcanza el objetivo por sí sola.**

**1. Prefetch inútil en el Header — implementado.** El logo y el selector de idioma se montan dos veces (mobile + desktop), y `next/Link` precarga por defecto todo lo que entra al viewport. El link "idioma actual" del selector apunta a la página en la que ya se está — precarga pura de desperdicio. Fix: `prefetch={false}` condicional cuando el href del link es la página actual (`Header.tsx`, `LanguageSwitcher.tsx`), sin tocar el prefetch real hacia el otro idioma (UX de cambio de idioma intacta). Verificado con `tsc`/ESLint limpios.

**2. Lazy-loading real de 3 componentes debajo del fold — implementado.** Investigado con la doc de esta versión de Next (`node_modules/next/dist/docs/01-app/02-guides/lazy-loading.md`): `next/dynamic` llamado directo desde un Server Component (como `page.tsx`) NO logra code-splitting real — hace falta un wrapper "use client" intermedio. Se crearon 3 wrappers (`BlogCarouselLazy.tsx`, `ClientsCarouselLazy.tsx`, `ContactFormLazy.tsx`, éste último cubre también el widget de Turnstile) y se cambiaron los imports en Home (ES/EN) y `ContactSection.tsx` (compartido por todas las páginas con formulario de contacto). `ssr: true` (default) — el contenido sigue completo en el HTML servido, cero regresión de SEO/CLS/accesibilidad sin JS.

**Validación con evidencia determinística, no con el score ruidoso de PSI:** se confirmó contra el HTML real servido (build de producción + `next start` local) que los 4 chunks de los 3 componentes ya NO aparecen en ningún `<script src>` eager de Home ni de /contacto — solo un `<link rel="preload" fetchPriority="low">` para el loader. El contenido SSR (texto del carrusel, formulario) se confirmó presente en el HTML de las dos páginas.

JS eager (bloqueante) antes/después — bytes reales de archivo, no estimados por PSI:

```
Home:      717.873 → 702.160 bytes (-15,7 KB, -2,2%)
/contacto: 703.552 → 696.322 bytes (-7,2 KB, -1,0%)
```

**Por qué es más modesto de lo esperado:** buena parte del peso de estos 3 componentes resultó ser código compartido (íconos, utilidades) que Turbopack igual necesita en el bundle eager para otras partes de la página — solo una fracción de sus ~40KB combinados se tradujo en bytes netos removidos del critical path.

**Techo realista sin tocar GTM:** con este resultado, el mayor peso individual que queda es Google Tag Manager (172KB transferidos, 73KB sin usar según `unused-javascript`) y el runtime de React/Next (~73KB gzip, no reducible sin abandonar el framework). Diferir o modificar la carga de GTM es una decisión de negocio (impacto en tracking), no una decisión técnica unilateral — queda pendiente de que Mariano decida si vale la pena explorarlo. Sin este cambio, +90 mobile de forma confiable no está garantizado solo con lo de hoy.

**Metodología descartada por poco confiable esta ronda:** se intentó medir el fix de prefetch contra un deploy de preview de Vercel (con `vercel project protection enable --protection-bypass` para que PSI, que no soporta headers/SSO, pudiera acceder) — 5 corridas antes/después dieron 49-93 de rango combinado, demasiado ruido (cache fría de un deployment recién creado) para separar señal del fix. La comparación de bytes reales (arriba) fue el método que sí dio una lectura confiable.

**Confirmado en producción tras el deploy (commit `34cc8f6`, pusheado a `master`):** Desktop sin cambios (98). Mobile, 5 corridas frescas: 86/67/70/73/74 (avg 74, mediana 73) — prácticamente igual al baseline de antes de los fixes (73/72/72/74/77, avg 74, mediana 73). Confirma lo que ya decía la comparación de bytes: -2,2% de JS eager no alcanza a moverse por encima del ruido normal de ±10-15 puntos de PSI.

**GTM investigado a pedido de Mariano ("explorá pero no rompas nada") — conclusión: ya está optimizado, no hay margen seguro.** Antes de tocar nada se revisó el historial de `GtmLoader.tsx`: dos commits previos del 5-6/9 (`cc6c0a5`, `01528e2`) ya separaron el shim inline de `dataLayer`/`gtag()` (eager, gratis, sin red) de la librería real de 166KB (diferida con `requestIdleCallback` + timeout de 2.5s, más un delay de 2s antes de pedir el idle slot, ajustado tras medir con Lighthouse local que colisionaba con el prefetch de `<Link>` de Next por compartir el primer slot idle). El código ya documenta por qué se descartó `next/script strategy="lazyOnload"` (espera `window.load` + `requestIdleCallback` sin timeout, sin garantía de cuándo carga) y por qué se descartó diferir por interacción del usuario (un page_view que rebota antes de cargar la librería es un dato perdido para siempre, a diferencia de Turnstile que sí puede saltarse del todo si nadie llega al formulario). Confirmado contra la traza de PSI de hoy: GTM carga recién en el ms ~2500, **después** de que se pinta el LCP (~2370ms) — no compite por el elemento crítico. Los 73KB "sin usar" que marca `unused-javascript` son código interno de la propia librería de Google, no recortable desde el sitio. No se tocó nada.

**De paso, se revisó `src/lib/fonts.ts` por la misma razón — mismo resultado: ya está optimizado.** De las 5 fuentes locales del sitio, las 2 que no son necesarias arriba del fold (`reey`, firma del fundador; `sora`, títulos de flip-cards en páginas de ciclo) ya tienen `preload: false` desde el 5/9, con el mismo razonamiento que se iba a proponer hoy ("compiten con la imagen del hero por ancho de banda temprano"). Las 3 que sí cargan eager (`gilmer` para H1/botones, `dmSans` para cuerpo de texto, `montserrat` para el botón del hero) son necesarias arriba del fold — diferirlas causaría FOUT/layout shift visible en el elemento LCP mismo, contraproducente. Todas ya son woff2-únicamente (evita el bug de `next/font/local` que precarga cada formato listado) y fuentes variables donde aplica (un solo archivo cubre todo el rango de pesos). Ninguna fuente sin explotar como palanca.

**Conclusión honesta de esta ronda: no hay un camino seguro identificado hoy para garantizar mobile +90 en el score de laboratorio de PSI.** Lo que queda de peso (runtime de React/Next ~73KB gzip, GTM ya diferido al máximo razonable, 3 fuentes necesarias arriba del fold) es costo estructural de la arquitectura actual, no fruta madura sin tocar — ya se pasó por acá en sesiones anteriores. Bajar más requeriría un cambio de arquitectura con trade-offs de UX reales (ej. menos interactividad cliente, sacrificar animaciones), no algo unilateral. **Contexto importante para no perseguir el número equivocado:** el score de PSI/Lighthouse es una métrica de *laboratorio* con throttling de CPU 4x simulado — el ranking real de Google usa datos de *campo* (CrUX, visitantes reales), que es exactamente lo que mide 1.25: 97,6% de los eventos INP reales del sitio son "good". El sitio ya está rindiendo bien donde Google realmente lo mide; el score de laboratorio es una señal más ruidosa y más dura de mover sin sacrificar UX.
...
En progreso · #7/#8/#9 hechos, #10 confirmado bloqueado por Cloudflare, solo queda #11 (copy en LinkedIn/Facebook)
...
**#9 — hecho (20/9).** Mariano importó la propiedad a Bing Webmaster Tools directo desde Google Search Console (sin verificación manual de DNS/meta tag/archivo) y confirmó el dominio dado de alta. El sitemap (`https://basecoresales.com/sitemap.xml`) no se importó automático — Bing mostró el aviso estándar "your data and reports are being processed, may take up to 48 hours"; Mariano lo cargó manual desde la sección Sitemaps para acelerar la indexación. Queda en estado "Processing" en Bing, esperable dentro de esas 48hs.

**#10 — resuelto el diagnóstico, sigue bloqueado, pero ahora se sabe por qué: no es un problema de herramientas, es Cloudflare.** Con el navegador ya liberado (20/9), Playwright navegó la página con éxito — pero Crunchbase devuelve un challenge de Cloudflare ("We must verify your session before you can proceed", HTTP 403, Ray ID `a3dcc9e7ea25d717`) incluso a un browser real con JS habilitado, no solo a WebFetch. Es un bot-check que ningún cliente automatizado va a poder pasar de forma confiable — no algo para reintentar con más intentos. Queda para que Mariano la revise él mismo, navegando con su browser normal (es una página pública, sin login).
...
| 1.28 | Performance con PageSpeed Insights (mobile/desktop, recurrente) | Pendiente · objetivo de +90 mobile cerrado el 21/9 |
