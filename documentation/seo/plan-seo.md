...
> - Última sincronización: 2026-09-19
> - Nota: sesión del 19/9 — a pedido de Mariano, ronda de 5 recomendaciones pendientes de la tarea 8.2 (visibilidad de marca "Base Core"), incluida #7 (mención en el footer), que había quedado explícitamente bloqueada el 14/9 y ahora se reabre a propósito. #8 (disambiguatingDescription en el JSON-LD) implementado y pusheado a producción (commit `fbcad13`); #7 y #11 con propuestas de copy listas, esperando que Mariano elija antes de tocar código o pegar texto; #9 confirmado que Bing Webmaster Tools permite importar la propiedad desde Search Console sin verificación manual; #10 (ficha de Crunchbase) inconcluso por límites de acceso a herramientas (navegador compartido ocupado, 403 ya conocido), a reintentar.
...
En progreso · #8 hecho (19/9), #7/#11 esperando tu confirmación de copy, #9 con instrucciones listas, #10 bloqueado por acceso
...
**#7 — reabierta por Mariano (ya no bloqueada), pidiendo que la mención viva en el footer. Propuesta de copy lista, sin implementar todavía:**

3 opciones de copy para Footer.tsx (ES/EN), esperando cuál elegir

```
Opción 1 (recomendada) — línea nueva y discreta en la sección 2,
separada de la línea de copyright:
  ES: "Base Core Sales también se conoce como Base Core."
  EN: "Base Core Sales is also known as Base Core."

Opción 2 — parentético dentro de la línea de copyright existente:
  ES: "Base Core Sales (también conocida como Base Core) © ..."
  EN: "Base Core Sales (also known as Base Core) © ..."
  Riesgo: alarga una línea ya centrada, wrap feo en mobile.

Opción 3 — leyenda junto al logo (Sección 1, no en la barra de
copyright), más visible pero en zona más cargada visualmente:
  ES: "Base Core Sales — también conocida como Base Core"
  EN: "Base Core Sales — also known as Base Core"
```
