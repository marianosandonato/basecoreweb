...
> - Nota: esta sesión cerró 1.37 (Turnstile en iOS, confirmado por Mariano en dos iPhones reales) mientras otra sesión trabajaba en paralelo sobre el mismo artifact — sacó el widget de revisión interactiva (duplicaba el archivo a ~190KB con una línea de +98.000 caracteres, forzó un `force` publish en esa sesión) y cerró 1.40 del todo (causa real: color de la foto, no el overlay — filtro CSS + overlay negro, solo en /marketing). Detalle completo de 1.40 ya está en el Historial Técnico SEO. 75/87 tareas. Esta sesión limpió una frase residual sobre 1.37 que había quedado contradictoria tras la fusión de ambos trabajos.
