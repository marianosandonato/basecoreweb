...
> - Nota: esta sesión cerró 1.25 (INP real de campo) y dio por cerrada del todo 1.28 (Performance PSI) — movida al Historial Técnico SEO, reemplazada por 5.9 (Mantenimiento continuo), chequeo mensual sin objetivo activo de score. 77/88 tareas.
...
📋 [Ver Historial Técnico SEO (detalle de las 62 tareas ya resueltas)](https://claude.ai/code/artifact/06216aa3-06d1-4a75-a16a-f76e134cfcd8)

78 / 88 tareas · 1 en progreso (8.2) · +4 bloqueadas (4.1, 4.3, 4.4, 4.5)
...
En progreso · #1/#2/#4/#7/#8/#9/#11 hechos, solo queda #10 (bloqueado, revisión manual de Mariano)
...
**#11 — hecho (21/9), con copy final distinto al propuesto el 18/9 — y una corrección de grafía en el camino.** En vez de las 2 opciones de frase de desambiguación breve, Mariano definió un perfil completo de LinkedIn personal centrado en "Base Core" como marca — el nombre aparece de entrada en el headline ("Fundador de Base Core"). **Corrección (21/9):** el copy pegado originalmente usaba "BaseCore" junto (una palabra) — exactamente la grafía que 8.1 había decidido evitar, por colisionar con BaseCore™ (geoceldas, marca registrada). Señalado al revisar la tarea antes de darla por cerrada; Mariano corrigió en LinkedIn a "Base Core" separado, consistente con 8.1 y con el resto del sitio (JSON-LD, `/llms.txt`, footer). Sin este chequeo, la tarea se hubiera dado por resuelta con una grafía que reabría el riesgo de marca de 8.1 sin que nadie lo notara.

**Alcance real de esta resolución, no el original de #11:** el pedido inicial (18/9) apuntaba a los perfiles de **empresa** ya creados (LinkedIn Company page, Facebook — los que el JSON-LD del sitio referencia vía `sameAs`), con una frase explícita "también conocida como Base Core Sales". Lo que se implementó es distinto: un perfil **personal** (LinkedIn de Mariano), sin esa frase de equivalencia explícita con "Base Core Sales". Cumple el objetivo de fondo (reforzar "Base Core" como término encontrable, y de paso el LinkedIn personal que 4.6 ya señalaba como débil) pero no es literalmente lo que #11 pedía — se cierra igual porque la estrategia completa de canales (empresa + personal) pasa a gestionarse desde el Plan de Marketing/Social, no porque este copy puntual sea la implementación original.

Copy final usado — LinkedIn personal (headline + about) y página de empresa:
...
**Ampliación de alcance (21/9):** este copy es parte de un trabajo más amplio de desarrollo de canales secundarios (LinkedIn empresa, Instagram, Facebook) que Mariano decidió el mismo día en 4.6 — el copy de #11 ya no vive solo como un fix puntual de desambiguación, es la base del perfil que se sigue desarrollando desde el [Plan de Marketing/Social](https://claude.ai/artifact/5nEdULGfDWCWES17cpptDp). Cualquier iteración futura de este copy (headline, about, posts) se gestiona ahí, no en el Plan de SEO.

**Para qué sirve:** que cualquiera que conozca el negocio como "Base Core"/"BaseCore" (sin el "Sales") pueda encontrar el perfil igual, dentro de lo que es realmente posible frente a la competencia por el término — y de paso, un perfil de LinkedIn completo en vez de uno vacío, que ya era un objetivo de 4.6.
...
El SEO no es un proyecto que se termina — esto es lo que se revisa de forma recurrente. 5.3, 5.4 y 5.5-5.8, ya cerrados, tienen detalle en el [Historial Técnico SEO](https://claude.ai/code/artifact/06216aa3-06d1-4a75-a16a-f76e134cfcd8). 5.9 es nueva (21/9), reemplaza a 1.28 (Fase 1, cerrada) como chequeo mensual de performance. 5.10 es 6.5 renumerada (21/9) — mismo mantenimiento mensual, movida de Fase 6.
