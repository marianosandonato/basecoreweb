...
> - Nota 25/9: 1.37 pasa a En progreso — causa confirmada en producción (temporizador de Turnstile.tsx), fix verificado en local y subido a un preview de Vercel sin commit; hueco de envíos sin token a cubrir con rate limiting en Cloudflare.
...
78 / 90 tareas · +5 bloqueadas (4.1, 4.3, 4.4, 4.5, 8.4)
...
1.37 — Turnstile (captcha de Cloudflare): cartel de error falso en desktop y fallas en mobile

En progreso · fix en preview 25/9

**Qué reporta Mariano (24/9):** en desktop, a veces aparece el cartel "No pudimos verificar la seguridad automáticamente — podés enviar el formulario igual" aunque la verificación sí se completó. En mobile, el captcha sigue sin funcionar bien.

**Antecedente:** se había cerrado el 20/9 con el arreglo del 18/9 (commit `a7b6948`): si el widget queda colgado 12 segundos, aparece el cartel y el formulario se puede enviar igual, con el honeypot como filtro. Detalle completo del cierre anterior en el [Historial Técnico SEO](https://claude.ai/code/artifact/06216aa3-06d1-4a75-a16a-f76e134cfcd8).

**Diagnóstico completo (25/9):** Cloudflare y el servidor están bien. El dominio pasa por el proxy de Cloudflare, no hay CSP que bloquee el widget, las claves están cargadas en Vercel (Production y Preview), el widget carga en producción sin error de dominio no autorizado, y `/api/contact` y `/api/ebook` rechazan un token inventado con "Verificación de seguridad fallida" (la clave secreta valida de verdad contra Cloudflare). Todas estas pruebas se hicieron sin enviar ningún email.

**Causa confirmada en producción:** se reprodujeron los dos síntomas con un Turnstile simulado que verifica a los 2 segundos. **(1)** En /contacto el cartel aparecía a los 12,6 s con el ✓ ya puesto: el temporizador no se cancelaba al verificar. No pasaba "a veces": pasaba en todas las visitas que se quedaban 12 s en la página. **(2)** En la Home, bajando al formulario a los 14 s, el cartel ya estaba visible antes de que el captcha se hubiera cargado: el temporizador contaba desde la carga de la página. En mobile es el mismo bug. Además, si el teléfono tarda más de 12 s en verificar, el cartel salía a mitad de la verificación.

**Fix (25/9, sin commit, en preview):** en `Turnstile.tsx`, el temporizador arranca cuando el widget se renderiza. Se cancela cuando llega el token y cuando el widget pasa a pedir un clic (`before-interactive-callback`). También se cancela en los callbacks de error y timeout, que siguen mostrando el cartel de inmediato. Verificado contra un build local con la clave de prueba de Cloudflare, en 6 escenarios: desktop y mobile en /contacto, /en/contact, /ebook y la Home con scroll tardío verifican sin cartel. Con el captcha colgado (simulado), el cartel sigue apareciendo a los 12 s y habilita el envío. Preview: `basecoreweb-oib2ymdm2-base-core.vercel.app`.

**Hueco de seguridad detectado:** desde el 18/9, `turnstile.ts` acepta envíos sin token, que es el fallback para iPhones con Private Relay. Se confirmó en producción que una solicitud sin token se salta el captcha y solo la frena el honeypot. Se mantiene el fallback para personas reales y se agrega una regla de **rate limiting en Cloudflare** (WAF → Rate limiting rules, 1 regla incluida en el plan Free) sobre los POST a `/api/contact` y `/api/ebook`. La crea Mariano en el panel, porque no hay acceso por API. También queda para Mariano confirmar que el widget esté en modo "Managed" (Turnstile → widget → Settings).

**Falta:** 1) Mariano prueba el preview en desktop y en su iPhone (si el widget muestra error de dominio en el preview, es porque `*.vercel.app` no está entre los hostnames del widget, y eso no refleja producción); 2) crear la regla de rate limiting; 3) con su OK, commit y deploy a producción, y verificación en vivo.

**Hecho cuando:** Mariano confirma en desktop y en su celular que el cartel aparece solo cuando la verificación falla de verdad, y que el formulario se envía y llega el email.

**Para qué sirve:** es el formulario de contacto y de descarga del e-book: el único camino por el que entran leads desde el sitio. Un cartel de error falso le dice al prospecto que algo no anda justo antes de enviar.
...
| 1.37 | Turnstile: cartel de error falso en desktop y fallas en mobile (reabierta 24/9, fix en preview 25/9) | En progreso |
