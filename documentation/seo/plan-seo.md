...
> - Nota: Mariano ya usó el widget de revisión sobre 1.39-1.41 — **1.39 y 1.41 quedaron OK; 1.40 (overlay del hero de /marketing) quedó "No"**, con nota "sigue estando muy azul, corregir" — el ajuste de `overlayOpacity` a 0.112 no fue suficiente, queda pendiente un nuevo intento antes de poder cerrarla. Este espejo no refleja el estado interactivo en sí (ver nota técnica de abajo), así que este veredicto queda documentado acá a mano.
> - Nota: 8.2 — #9 hecho (Mariano importó la propiedad a Bing Webmaster Tools desde GSC y cargó el sitemap manual, "Processing" normal hasta 48hs). De la ronda de 5 recomendaciones de 8.2 solo falta #11 (pegar el copy de desambiguación en LinkedIn/Facebook), pendiente de Mariano.
> - Nota anterior (mismo día): se agregó revisión interactiva (OK/No/Sin revisar + aclaración + resumen + botón "Guardar revisión") a los 3 hallazgos de UI/UX de 1.39-1.41, a pedido de Mariano, siguiendo el mismo patrón que usaba el artifact discontinuado "Auditoría Final Base Core". Mariano puede marcar su veredicto por hallazgo directo en el artifact; la sesión principal lee esos veredictos la próxima vez que abra el artifact y actualiza el plan (mueve a Hecho, documenta en el Historial) según corresponda.
...
71 / 87 tareas · 2 en progreso (1.25, 8.2) · +4 bloqueadas (4.1, 4.3, 4.4, 4.5) · +3 pendientes de revisión en Fase 1 (1.39-1.41, implementadas 19-20/9)
...
**Fix:** se saca la condición — el spacer ahora es incondicional. /preventa mide 106.4px, igual que /tecnologia, confirmado en vivo (Playwright, ES y EN, 1280px y 390px). Commit `9ccbf6f`.
...
**Fix:** `overlayOpacity` explícito de 0.112 (20% relativo menos que el default de `PageHero`, 0.14) solo en las dos versiones de /marketing (ES/EN) — ningún otro caller de `PageHero` tocado. Verificado visualmente con Playwright y confirmado en el HTML servido en vivo de basecoresales.com/marketing (el valor 0.112 está presente). Commit `f2fdf02`.

**Para qué sirve:** que la imagen del hero se vea, no solo el tinte de marca encima.
...
**Causa raíz:** un `:hover`/`:focus-within` de CSS y un estado `open` de React controlaban el mismo visual sin coordinarse — el hover forzaba el dorso sin importar lo que un clic acabara de setear, y al sacar el cursor el CSS soltaba pero el estado de React quedaba pisado (se trababa en dorso). En mobile, el primer tap disparaba a la vez un focus/hover sintético y el propio clic, compitiendo por el mismo estado — de ahí que el primer tap "no hiciera nada" y recién el segundo funcionara.

**Fix:** `useFlipTeaser.ts` reemplaza el trigger de hover por estado real: `mouseenter`/focus abre, `mouseleave`/blur siempre cierra (sin condición), clic togglea — todo gateado a punteros hover-capable via `matchMedia`, así el tap en touch sólo dispara `onClick`, sin competencia posible. Las cards con href (sólo "Ciclos" en Home) mantienen su comportamiento CSS-only sin cambios.

**Verificación:** comportamiento exacto verificado en desktop con Playwright (Home, /marketing, /preventa) — hover flipea, mouseleave siempre vuelve al frente incluso después de un clic, clics repetidos togglean. La emulación de tap táctil real no está disponible en las herramientas de Playwright de este entorno (`hover:hover` sigue en `true` a ancho mobile) — el mecanismo de mobile se verificó por revisión de código: el gating elimina por construcción cualquier camino que no sea el clic para mutar estado en touch. Commit `4561e89`.

**Para qué sirve:** que la interacción principal de las flip cards (todo el sitio las usa para mostrar detalle de servicios) funcione de forma predecible en el primer intento, en desktop y mobile.
