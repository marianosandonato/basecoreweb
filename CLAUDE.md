...
- **BaseCoreWeb: SEO y Performance** (antes "Plan de SEO de Base Core", renombrado el 20/9 a pedido de Mariano) — plan de fases (1 a 8), qué está Hecho/Pendiente/Bloqueado en cada una. Desde el 20/9 también trackea hallazgos de UI/UX del sitio (Fase 1, 1.39 en adelante) que Mariano reporta navegando el sitio — rol que antes cubría la Auditoría Final, hoy archivada y cerrada del todo (ver "Documentos archivados"). Artifact (fuente de verdad, solo accesible desde la sesión principal — confirmado el 3/9 que la tool `Artifact` no está disponible para sub-agentes, es una restricción de plataforma): https://claude.ai/artifact/XPrZBTCe2b7tvbzzNuf1GT — los agentes (`web-lead`, `seo-marketing`, `performance`) deben leer en su lugar el espejo: `documentation/seo/plan-seo.md`.
...
**Desde el 12/9, el Plan de SEO es también el único documento de trabajo para performance** — absorbió el rol operativo que tenía el artifact Performance Web (archivado, ver "Documentos archivados" más abajo): la Fase 1 trackea ahí las tareas de performance (1.23 en adelante), con el mismo criterio Hecho/Pendiente/Bloqueado que el resto del plan. Metodología: para cualquier tarea de SEO o de performance, invocar al agente correspondiente (`seo-marketing` o `performance`) para ejecutar el trabajo puntual, pero el estado de qué falta vive únicamente en el Plan de SEO — no reabrir Performance Web como tablero paralelo, y no crear un tablero de performance nuevo si este llegara a archivarse también: la próxima vez, la tarea que reemplace este rol hereda esta misma regla.
...
## Documento de Marketing vigente

- **Marketing Strategy Basecore** — plan único de marketing y desarrollo comercial de Base Core: accionables por fase (LinkedIn, contenido, prueba social, demanda/nurturing, paid media, SEO/AI Search puente, marca) arriba, diagnóstico/ICP/posicionamiento/pilares de contenido abajo. Reemplaza y renombra el artifact "Plan Social Media" (4/9) — absorbió esa auditoría de `social-content` como parte de su Fase 2. Artifact (fuente de verdad, solo accesible desde la sesión principal, mismo límite de plataforma que los artifacts de SEO): https://claude.ai/artifact/5nEdULGfDWCWES17cpptDp — los agentes (`social-content`, `seo-marketing`, `web-lead`) deben leer en su lugar el espejo: `documentation/marketing/marketing-strategy.md`.

**Regla de sincronización:** igual que con los documentos de SEO — el espejo es la única vía de acceso real para los agentes, mantenerlo al día no es opcional. Cualquier publish a este artifact (solo la sesión principal) debe ir acompañado, en el mismo momento, de actualizar su espejo (`markitdown <html-local> -o documentation/marketing/marketing-strategy.md`, conservando la nota de "Espejo de trabajo" al principio). **Verificación de frescura:** antes de cualquier tarea de marketing, correr `Artifact` con `action: "list"` y comparar el `last-updated` real contra la fecha de "Última sincronización" del espejo — si el artifact es más nuevo, resincronizar antes de delegar a un agente. Aplica el mismo criterio Camino B (un dueño por dato) frente al Plan de SEO, al Mapa de Keywords y a la Auditoría de Marca: este documento linkea el detalle técnico de SEO/AI-search/naming en vez de repetirlo.
...
- **Auditoría Final Base Core** — https://claude.ai/code/artifact/dc919209-ee50-4b8a-9588-1e176c511faf — archivada el 19/9. Auditoría integral de spacing, flip cards, responsive, UX/UI, SEO, performance y accesibilidad corrida el 14/9 (25 hallazgos, 3 P1). Cerrada del todo el 18-19/9: 9 hallazgos implementados con código real y verificados, 10 migrados al Plan de SEO (su dueño correcto para SEO/performance, tareas 1.29-1.37, 3.11-3.12, 1.36, todas resueltas), 2 verificados como falsos positivos, 1 descartado por desproporción de alcance, 3 explícitamente "no tocar" por decisión de Mariano — el último cabo suelto (nota sin retomar sobre márgenes del cajón "Etapas" en /preventa, /venta, /posventa) se cerró el 19/9. Detalle completo del proceso de cierre, fase por fase, en `documentation/PLAN-AUDITORIA-FINAL.md`. No tiene espejo en `documentation/seo/` (no es uno de los tres artifacts SEO con esa regla) — accesible solo desde la sesión principal.
...
- **Performance Web** — https://claude.ai/code/artifact/63c7e1d6-16c6-4b2c-8259-186ea93a6929 — archivado el 12/9. Todo el seguimiento operativo de performance (qué está pendiente/bloqueado/en pausa) se mudó al Plan de SEO, Fase 1, tareas 1.23 en adelante — ese es el único lugar a mirar para saber qué falta. Este artifact sigue siendo el registro histórico completo de cómo se resolvió cada cosa de performance ya hecha (mediciones reales, commits, método, hallazgos descartados) — consultarlo para el detalle técnico de una tarea ya cerrada, nunca para saber qué sigue abierto. No tiene espejo en `documentation/seo/` (no es uno de los tres artifacts SEO con esa regla) — accesible solo desde la sesión principal, igual que los demás artifacts de este documento.
